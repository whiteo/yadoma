...
public class UserService implements UserDetailsService {
     ...
     * Initializes the containerIds list if it's null.
     *
    ...
    public void addContainerToUser(String containerId, User user) {
        if (user.getContainerIds() == null) {
            user.setContainerIds(new java.util.ArrayList<>());
        }
        user.getContainerIds().add(containerId);
        User savedUser = repository.save(user);
        log.info("Added container {} to user {}. User now has {} containers: {}",
                containerId, user.getId(), savedUser.getContainerIds().size(), savedUser.getContainerIds());
    }
